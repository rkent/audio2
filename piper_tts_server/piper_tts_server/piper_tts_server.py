...
class PiperTtsServerNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('model', 'en_US-bryce-medium')
        ...
        logging.info(f'Piper TTS server starting with model="{model}"')
        ...
        def app_synthesize() -> bytes:
            ...
            req_model_id = data.get('voice', '')
            if not req_model_id and default_model_id:
                req_model_id = default_model_id
                _LOGGER.info(f'Using default voice: {req_model_id}')
