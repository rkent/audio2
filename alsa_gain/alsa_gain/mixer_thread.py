...
class MixerThread:
    ...
    @property
    def ready(self) -> bool:
        """Indicate if the mixer is initialized and ready. Thread-safe."""
        with self.lock:
            return self.mixer is not None

    @property
    def volume(self) -> list[int]:
        """Get current volume percentage in a thread-safe manner."""
        with self.lock:
            return self._volume
        ...
        if self.mixer:
            ...
            try:
                ...
                if not self.mute_warned:
                    logger.warning(
                        'Mixer does not support getmute(); '
                        'assuming unmuted.'
                    )
            ...
            logger.info(
                'Mixer event detected! New Volume: %s%%, Muted: %s',
                self._volume,
                self._muted
            )
    ...
    def register_mixer(self, control: str, device: str) -> None:
        """
        Initialize and register the ALSA Mixer object.
        ...
        if not (valid_playback or valid_capture):
            ...
            logger.info(
                f'Available playback devices: {playback_devices}'
            )
            logger.info(
                f'Available capture devices: {capture_devices}'
            )
            return
        ...
        try:
            ...
            logger.error(
                f'Error retrieving available mixers, '
                f"device '{device}' probably has no controls"
            )
            # which devices are available with controls?
            # This may display errors, unsuccessful in suppressing.
            logger.info(
                'Checking which devices have available mixer controls. '
                'This may show ALSA lib errors, ignore them'
            )
            valid_devices = set()
            ...
            for capture_device in capture_devices:
                try:
                    ...
                    pass
            logger.info(
                f'Devices with available mixer controls: {valid_devices}'
            )
            return
        if control not in avail_controls:
            logger.warning(
                f"Invalid control '{control}' for device '{device}'."
            )
            try:
                avail_controls = aa.mixers(device=device)
                logger.info(
                    f"Available mixer controls on device '{device}': "
                    f'{avail_controls}'
                )
            except aa.ALSAAudioError as e2:
                logger.error(
                    f"Error retrieving available mixers, device '{device}': "
                    f'{e2}'
                )
            return
        try:
        ...
        logger.info(
            f"Monitoring '{self.mixer.mixer()}' control on device "
            f"'{device}'."
        )
        ...
        if not descriptors:
            logger.warning(
                'Could not get poll descriptors for the mixer. Exiting.'
            )
            return
        ...
        # Register the file descriptor with selector and callback
        # The 'data' field stores the mixer object.
    ...
    def set_alsa_mute(self, mute: bool, channel: int | None = None) -> None:
        """Set ALSA mute state for channel or all channels."""
        if self.mixer:
            logger.info('Set alsa mute to %s channel %s', mute, channel)
            try:
                ...
                if not self.mute_warned:
                    logger.warning(
                        'Mixer does not support setmute(); '
                        'cannot set mute state.'
                    )
                    self.mute_warned = True
        else:
            logger.warning('Mixer object is not initialized; cannot set mute.')
    ...
    def set_alsa_volume(self, volume: int, channel: int | None = None) -> None:
        """Set ALSA volume for channel or all channels."""
        if self.mixer:
            logger.info('Set alsa volume to %s channel %s', volume, channel)
            try:
                if type(channel) is int:
    ...
    def handle_mixer_event(self, _) -> None:
        """Handle mixer event callback."""
        # Read the event to clear the ALSA buffer and get new volume/status
    ...
    def handle_message_event(self, _) -> None:
        """Handle pipe data callback when read end is ready."""
        ...
        if not data:
            ...
            if self.shutdown_event.is_set():
                logger.info(
                    'Main thread closed the pipe. Unregistering selector.'
                )
                self.sel.unregister(read_fd)
                os.close(read_fd)
            else:
                logger.info(
                    'Shutdown event not set, continuing message handler.'
                )
            ...
            if not self.mixer:
                logger.warning(
                    'Mixer object is not initialized; cannot process message.'
                )
                return
                        ...
                        if all(item == volume[0] for item in volume):
                            ...
                            for channel in range(len(volume)):
                                self.set_alsa_volume(
                                    int(volume[channel]), channel
                                )
                    else:
                        self.set_alsa_volume(int(volume))
                else:
                    logger.warning(
                        'Unknown command received: %s', message.get('command')
                    )
            # After processing, update mixer status (no event seems to fire)
            self.update_mixer()

    def run(self):
        """Monitor or initiate ALSA mixer events in thread worker."""
...
if __name__ == '__main__':
    ...
    try:
        while True:
            logger.info('Main thread doing other work %d', count)
            time.sleep(2)
            count += 1
            if count % 5 == 0:
